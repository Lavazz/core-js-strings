...
function isString(value) {
  ...
  const primitive = typeof value === 'object' ? value.valueOf() : value;
...
function removeFirstOccurrences(str, value) {
  ...
  return str.substring(0, indexStart).concat(str.substring(indexEnd));
}
...
function removeLastOccurrences(str, value) {
  const lastIndexStart = str.lastIndexOf(value);
  if (lastIndexStart < 0) {
    return str;
  }
  const indexEnd = lastIndexStart + value.length;
  return str.substring(0, lastIndexStart).concat(str.substring(indexEnd));
}
...
function formatTime(minutes, seconds) {
  return seconds.toString().padStart(5, `0${minutes}:`);
}
...
function findLongestWord(sentence) {
  const sentenceAr = sentence.split(' ');
  const lengthAr = sentenceAr.map((word) => word.length);
  let largest = 0;
  let largestIndex = 0;
  lengthAr.map((wordLength, i) => {
  ...
  return sentenceAr[largestIndex];
}
...
function reverseWords(str) {
  ...
  const reversed = strAr.map((word) => [...word].reverse().join(''));
...
function invertCase(str) {
  ...
  const transformWords = strArray.map((word) => {
    let invertLetter;
    const invertWord = word.split('').map((letter) => {
...
function encodeToRot13(str) {
  ...
  const shiftedArr = strArr.map((letter) => {
    if (normalAlphabet.includes(letter)) {
      const index = normalAlphabet.indexOf(letter);
